...
const ResultScreen = ({ route }) => {
  ...
  const GPT_API_KEY = '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'; // Replace with your GPT API key
  const CSE_API_KEY = 'AIzaSyDi1BnDCsFPHZy7rie-OdVA-LNIKgyu_NA'; // Replace with your actual Google API key
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response.data.class) {
          ...
          if (acceptableDiseases.includes(predictedClass)) {
            ...
            await fetchDiseaseInfoWithGPT("eggplant " + predictedClass);
            setLoading(false);
          } else {
    ...
    const fetchDiseaseInfoWithGPT = async (disease) => {
      try {
        // Use ChatGPT to get disease info
        const gptResponse = await axios.post(
          'https://api.openai.com/v1/chat/completions',
          {
            model: 'gpt-3.5-turbo', // Or gpt-4 if available
            messages: [
              { role: 'system', content: 'You are an expert on eggplant diseases.' },
        ...
        // Translate ChatGPT response to Urdu
        const translatedResponse = await translateText(gptMessage, 'ur');

        setDiseaseInfo({
          en: gptMessage,
          ur: translatedResponse || 'No information available for this disease.',
        });
      } catch (error) {
        // If GPT fails, fallback to Google CSE
        await fetchDiseaseInfoFromCSE(disease);
      }
    };

    const fetchDiseaseInfoFromCSE = async (disease) => {
      try {
        const searchResponse = await axios.get(
          `https://www.googleapis.com/customsearch/v1?q=${disease}&cx=${CSE_ID}&key=${CSE_API_KEY}`
        );
    ...
    const translateText = async (text, targetLanguage) => {
      try {
        const response = await axios.post(
          `https://translation.googleapis.com/language/translate/v2?key=${CSE_API_KEY}`,
          {
            q: text,
...
const styles = StyleSheet.create({
  ...
  text: {
    ...
    color: 'snow',
    fontWeight:'100',
    textAlign: 'justify', // Align text to be justified
